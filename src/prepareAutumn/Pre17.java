...
public class Pre17 {
    ...
    public List<String> letterCombinations1(String digits) {
        ...
        List<String> ans = new ArrayList<>();
        travel(digits, new StringBuilder(), 0, ans, map);
        return ans;
    }

    private void travel(String digits, StringBuilder sb, int index, List<String> ans, Map<Character, char[]> map) {
        if (index == digits.length()){
            ans.add(digits.toString());
            return;
        }
        ...
        for (char letter : chars){
            sb.append(letter);
            travel(digits, sb, index + 1, ans, map);
            sb.deleteCharAt(sb.length() - 1);
        }
    }
